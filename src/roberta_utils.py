...
def functional_roberta_for_classification(fast_weights, config, input_ids=None, attention_mask=None, token_type_ids=None,
                                          position_ids=None, head_mask=None, inputs_embeds=None, encoder_hidden_states=None,
                                          encoder_attention_mask=None, labels=None, is_train=True):
    ...
    x = F.linear(x,
                 fast_weights['classifier.dense.weight'],
                 fast_weights['classifier.dense.bias'])
    x = torch.tanh(x)
    x = F.dropout(x, dropout_rate, training=is_train)
    logits = F.linear(x,
...
def functional_roberta(fast_weights, config, input_ids=None, attention_mask=None, token_type_ids=None,
    ...
    outputs = (sequence_output, pooled_outputs) + encoder_outputs[1:]
    return outputs
...
def functional_pooler(fast_weights, sequence_output):
    ...
    pooled_output = F.linear(first_token_tensor,
                             fast_weights['roberta.pooler.dense.weight'],
                             fast_weights['roberta.pooler.dense.bias'])
...
def functional_out_attention(fast_weights, config, layer_idx,
    ...
    hidden_states = F.layer_norm(hidden_states + input_tensor, [config.hidden_size],
                                 weight=fast_weights[
                                     'roberta.encoder.layer.' + layer_idx + '.attention.output.LayerNorm.weight'],
                                 bias=fast_weights[
                                     'roberta.encoder.layer.' + layer_idx + '.attention.output.LayerNorm.bias'],
                                 eps=config.layer_norm_eps)
